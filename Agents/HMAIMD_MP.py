...
class HMAIMD_Agent(object):
    ...
    def step(self):
        ...
        nodes_start_episode_num = max_buffer_number * 0.5
        reward_start_episode_num = max_buffer_number * 2
        ...
        with tqdm(total=self.environment.max_episode_length) as my_bar:
            ...
            while not self.done:  # when the episode is not over
                ...
                if self.time_for_actor_of_sensor_nodes_and_edge_node_to_learn(nodes_start_episode_num, during_episode_number, update_every_n_steps):
                    ...
                    list_sensor_nodes_observations = []
                    list_sensor_nodes_actions = []
                    list_edge_node_observations = []
                    list_next_sensor_nodes_observations = []
                    ...
                    for _ in range(self.hyperparameters["actor_nodes_learning_updates_per_learning_session"]):
                        ...
                        list_sensor_nodes_observations.append(sensor_nodes_observations)
                        list_sensor_nodes_actions.append(sensor_nodes_actions)
                        list_edge_node_observations.append(edge_node_observations)
                        list_next_sensor_nodes_observations.append(next_sensor_nodes_observations)

                    for sensor_node_index in range(self.environment.experiment_config.vehicle_number):
                        actor_loss = pool.apply_async(self.actor_sensor_node_from_list_to_learn, 
                                                        args=(
                                                            [sensor_node_index for _ in range(len(list_sensor_nodes_observations))],
                                                            list_sensor_nodes_observations,
                                                            list_sensor_nodes_actions,
                                                            list_next_sensor_nodes_observations
                                                            )).get()
                        actor_loss_of_sensor_nodes[sensor_node_index] = actor_loss

                    actor_loss_of_edge_node = pool.apply_async(self.actor_edge_node_from_list_to_learn, 
                                                        args=(
                                                            list_edge_node_observations,
                                                            list_sensor_nodes_actions,
                                                            list_next_sensor_nodes_observations
                    ...
                    list_edge_node_observations = []
                    list_sensor_nodes_actions = []
                    list_edge_node_actions = []
                    list_sensor_nodes_rewards = []
                    list_edge_node_rewards = []
                    list_next_sensor_nodes_observations = []
                    list_next_edge_node_observations = []
                    list_done = []
                    ...
                    for _ in range(self.hyperparameters["critic_nodes_learning_updates_per_learning_session"]):
                        ...
                        list_edge_node_observations.append(edge_node_observations)
                        list_sensor_nodes_actions.append(sensor_nodes_actions)
                        list_edge_node_actions.append(edge_node_actions)
                        list_sensor_nodes_rewards.append(sensor_nodes_rewards)
                        list_edge_node_rewards.append(edge_node_rewards)
                        list_next_sensor_nodes_observations.append(next_sensor_nodes_observations)
                        list_next_edge_node_observations.append(next_edge_node_observations)
                        list_done.append(dones)

                    for sensor_node_index in range(self.environment.experiment_config.vehicle_number):
                        critic_loss = pool.apply_async(self.critic_sensor_node_from_list_to_learn, 
                                                        args=(
                                                            [sensor_node_index for _ in range(len(list_sensor_nodes_observations))],
                                                            list_sensor_nodes_observations,
                                                            list_sensor_nodes_actions,
                        ...
                        critic_loss_of_sensor_nodes[sensor_node_index] = critic_loss
                    
                    critic_loss_of_edge_node = pool.apply_async(self.critic_edge_node_from_list_to_learn, 
                                                        args=(
                                                            list_edge_node_observations,
                                                            list_sensor_nodes_actions,
                                                            list_edge_node_actions,
                    ...
                    time_end = time.time()
                    print("time_for_actor_and_critic_of_sensor_nodes_and_edge_node_to_learn: ", time_end - time_start)

                for index in range(self.environment.experiment_config.vehicle_number):
                    average_actor_loss_of_sensor_nodes[index] = actor_loss_of_sensor_nodes[index]
                    average_critic_loss_of_sensor_nodes[index] = critic_loss_of_sensor_nodes[index]
                average_actor_loss_of_edge_node = actor_loss_of_edge_node
                average_critic_loss_of_edge_node = critic_loss_of_edge_node
    ...
    def conduct_action(self):
        ...
        self.next_sensor_nodes_observation, self.next_edge_node_observation, self.next_reward_observation, \
            ...
            sum_queuing_time, sum_transmitting_time, sum_service_time, sum_service_rate, sum_received_data_number, \
            sum_required_data_number = self.environment.step(self.action)
    ...
    def actor_sensor_node_from_list_to_learn(
        self,
        list_sensor_node_index,
        list_sensor_nodes_observations,
        list_sensor_nodes_actions,
        list_next_sensor_nodes_observations
    ):
        one_time_average_actor_loss_of_sensor_node = 0
        for i in range(len(list_sensor_nodes_observations)):
            sensor_node_index = list_sensor_node_index[i]
            sensor_nodes_observations = list_sensor_nodes_observations[i]
            sensor_nodes_actions = list_sensor_nodes_actions[i]
            next_sensor_nodes_observations = list_next_sensor_nodes_observations[i]

            actor_loss_of_sensor_node = self.actor_sensor_node_to_learn(
                sensor_node_index,
                sensor_nodes_observations,
                sensor_nodes_actions,
                next_sensor_nodes_observations
            )
            one_time_average_actor_loss_of_sensor_node += actor_loss_of_sensor_node
        try:
            one_time_average_actor_loss_of_sensor_node /= len(list_sensor_nodes_observations)
        except:
            pass
        return one_time_average_actor_loss_of_sensor_node

    def actor_sensor_node_to_learn(
        self,
        sensor_node_index,
        ...
        actor_loss_of_sensor_node_value = actor_loss_of_sensor_node.item()
        ...
        return actor_loss_of_sensor_node_value

    def actor_edge_node_from_list_to_learn(
        self,
        list_edge_node_observations,
        list_sensor_nodes_actions,
        list_next_sensor_nodes_observations
        ...
        for i in range(len(list_edge_node_observations)):
            edge_node_observation = list_edge_node_observations[i]
            sensor_nodes_actions = list_sensor_nodes_actions[i]
            next_sensor_nodes_observations = list_next_sensor_nodes_observations[i]

            actor_loss_of_edge_node = self.actor_edge_node_to_learn(
        ...
        try:
            one_time_average_actor_loss_of_edge_node /= len(list_edge_node_observations)
        except:
            pass
        return one_time_average_actor_loss_of_edge_node
    ...
    def critic_sensor_node_from_list_to_learn(
        self,
        list_sensor_node_index,
        list_sensor_nodes_observations,
        list_sensor_nodes_actions,
        ...
        one_time_average_critic_loss_of_sensor_node = 0
        
        for i in range(len(list_sensor_nodes_observations)):
            sensor_node_index = list_sensor_node_index[i]
            sensor_nodes_observations = list_sensor_nodes_observations[i]
            sensor_nodes_actions = list_sensor_nodes_actions[i]
            sensor_nodes_rewards = list_sensor_nodes_rewards[i]
            next_sensor_nodes_observations = list_next_sensor_nodes_observations[i]
            dones = list_dones[i]

            critic_loss_of_sensor_node = self.critic_sensor_node_to_learn(
                sensor_node_index,
                sensor_nodes_observations,
                sensor_nodes_actions,
                sensor_nodes_rewards,
                next_sensor_nodes_observations,
                dones
            )

            one_time_average_critic_loss_of_sensor_node += critic_loss_of_sensor_node
        try:
            one_time_average_critic_loss_of_sensor_node /= len(list_sensor_nodes_observations)
        except:
            pass
        
        return one_time_average_critic_loss_of_sensor_node

    def critic_sensor_node_to_learn(
        self,
        sensor_node_index,
        sensor_nodes_observations: list,
        sensor_nodes_actions: list,
        ...
        critic_loss_of_sensor_node_value = critic_loss_of_sensor_node.item()
        ...
        return critic_loss_of_sensor_node_value

    def critic_edge_node_from_list_to_learn(
        self,
        list_edge_node_observations,
        list_sensor_nodes_actions,
        list_edge_node_actions,
        ...
        for i in range(len(list_edge_node_observations)):
            edge_node_observations = list_edge_node_observations[i]
            sensor_nodes_actions = list_sensor_nodes_actions[i]
            edge_node_actions = list_edge_node_actions[i]
            edge_node_rewards = list_edge_node_rewards[i]
            next_sensor_nodes_observations = list_next_sensor_nodes_observations[i]
            next_edge_node_observations = list_next_edge_node_observations[i]
            dones = list_dones[i]

            critic_loss_of_edge_node = self.critic_edge_node_to_learn(
        ...
        try:
            one_time_average_critic_loss_of_edge_node /= len(list_edge_node_observations)
        except:
            pass
        
        return one_time_average_critic_loss_of_edge_node
